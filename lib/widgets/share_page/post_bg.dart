...
class PostTimeBannerInfo{
  ...
  init() async {
    leftBannerImage = await ShareImageTool.getImageWithAsset("assets/post_left_banner.png");
    rightBannerImage = await ShareImageTool.getImageWithAsset("assets/post_right_banner.png");
  }
  ...
  paint(Canvas canvas, Size size){
    ...
    Offset rightOffSet = Offset(ScreenUtil.screenWidthDp - 80 - rightBannerWidth +15, size.height - 130);
    ...
    if (!TextUtils.isEmpty(timeInfo)) {
      ...
      canvas.drawImageRect(
        rightBannerImage,
        Offset(0.0, 0.0) &
            Size(rightBannerImage.width.toDouble(), rightBannerImage.height.toDouble()),
      ...
      ui.Paragraph endTimeParagraph = endTimeParagraphBuilder.build()
        ..layout(ui.ParagraphConstraints(width:rightBannerWidth,));
      canvas.drawParagraph(endTimeParagraph, Offset(rightOffSet.dx+5, leftOffSet.dy+9.5));
      // 左边红色
      canvas.drawImageRect(
        leftBannerImage,
        Offset(0.0, 0.0) &
            Size(leftBannerImage.width.toDouble(), leftBannerImage.height.toDouble()),
        leftOffSet & Size(ScreenUtil.screenWidthDp - 80 - rightBannerWidth + 15, 45),
        paint);
      // 红色文字
      ...
      ui.Paragraph infoParagraph = infoParagraphBuilder.build()
        ..layout(ui.ParagraphConstraints(width: ScreenUtil.screenWidthDp - 80 - rightBannerWidth + 15,));
      canvas.drawParagraph(infoParagraph, Offset(leftOffSet.dx+10, leftOffSet.dy+12));
    }else{
      // 左边红色
      canvas.drawImageRect(
        leftBannerImage,
        Offset(0.0, 0.0) &
            Size(leftBannerImage.width.toDouble(), leftBannerImage.height.toDouble()),
        leftOffSet & Size(ScreenUtil.screenWidthDp - 80+3, 45),
        paint);
      // 红色文字
      ui.ParagraphBuilder infoParagraphBuilder = ui.ParagraphBuilder(
        ui.ParagraphStyle(
        ...
        ..addText("全球精品 | 正品保障 | 售后无忧");
      ui.Paragraph infoParagraph = infoParagraphBuilder.build()
        ..layout(ui.ParagraphConstraints(width: ScreenUtil.screenWidthDp - 80+3 - 20,));
      canvas.drawParagraph(infoParagraph, Offset(leftOffSet.dx+10, leftOffSet.dy+12));
    }
    canvas.restore();
  }
...
class PostBottomInfo{
  ...
  String info;
  String qrCode;
  PostBottomInfo({this.price="", this.crossedPrice="", this.info="", this.qrCode="",});
  init() async {
    image = await ShareImageTool.getImageWithQRCode(qrCode: this.qrCode, size: qrSize.width);
  }
  ...
  paint(Canvas canvas, Size size){
    ...
    ui.ParagraphBuilder infoParagraphBuilder = ui.ParagraphBuilder(
      ...
      ..addText(info);
    ui.Paragraph infoParagraph = infoParagraphBuilder.build()
      ..layout(ui.ParagraphConstraints(width: ScreenUtil.screenWidthDp-150));
    canvas.drawParagraph(infoParagraph, Offset(textLeft, size.height - 50));
    //
    // 二维码
    Offset qrOffset = Offset(ScreenUtil.screenWidthDp - 90 - 25, size.height - 75);
    ...
    canvas.drawParagraph(qrTextParagraph, Offset(qrOffset.dx-10, size.height - 20));
    Rect qrCodeWrap = qrOffset & Size(50, 50);
    Paint paint = new Paint();
    canvas.drawImageRect(
      image,
      Offset(0.0, 0.0) & qrSize,
      qrCodeWrap,
      paint);
    canvas.restore();
  }
...
class PostUserImage{
  ...
  init() async {
    image = await ShareImageTool.getImageWithAsset(AppImageName.recook_icon_120);
  }
  ...
  paint(Canvas canvas) async {
    ...
    canvas.drawImageRect(
        image,
        Offset(0.0, 0.0) & Size(image.width.toDouble(), image.height.toDouble()),
        screenWrap,
        paint);
    canvas.restore();
  }
