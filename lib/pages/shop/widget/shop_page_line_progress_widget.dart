...
class ShopPagePopPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    Paint p = new Paint();
    p.color = Colors.black38.withAlpha(140);//画笔颜色
    // p.color = Colors.red;
    p.strokeWidth = 1;
    ...
    // 右上角
    Rect rectRightTop = Rect.fromCircle(center: Offset(size.width-5, 10), radius: 5);
    path.arcTo(rectRightTop, 3.14*1, 3.14*1, false);
    path.lineTo(size.width, size.height-5);
    // 右下角
    Rect rectRightBottom = Rect.fromCircle(center: Offset(size.width-5, size.height-5), radius: 5);
    path.arcTo(rectRightBottom, 3.14*1.5, 3.14*1, false);
    path.lineTo(5, size.height);
    // 左下角
    Rect rectLeftBottom = Rect.fromCircle(center: Offset(5, size.height-5), radius: 5);
    path.arcTo(rectLeftBottom, 3.14*.5, 3.14*.5, false);
    canvas.drawPath(path, p);

    TextPainter textPainter = TextPainter(textAlign: TextAlign.start ,maxLines: 2, textDirection: TextDirection.ltr);
    textPainter.text = TextSpan(
      children: [
        TextSpan(text: text, style: TextStyle(color: Color(0xfffbffff), fontSize: 10)),
      ]
    );
...
class ShopPopViewModel {
  ...
  bool leftOrRight;
  String popTitle;
  ShopPopViewModel(this.startX, this.left, this.top, this.width, this.height, this.offstage, {this.leftOrRight = true, this.popTitle = "", this.nodeIndex=-1});
}
...
class ShopPageLineProgressWidget extends StatefulWidget {
  ...
  ShopPageLineProgressWidget({Key key, this.camelTitle="", this.percent = 0, this.width = 100, this.height = 50, this.nodeCount = 5, this.camelPopTitle="", this.nodeTitleList}) : super(key: key);

  @override
  ShopPageLineProgressWidgetState createState() => ShopPageLineProgressWidgetState();
}

class ShopPageLineProgressWidgetState extends State<ShopPageLineProgressWidget> {
  ...
  updateView({percent}){
    ...
    start = start<10?10:start>widget.width-10?widget.width-10:start;
    ...
    _normalPopViewModel = ShopPopViewModel(start-normalLeft , normalLeft, _allBoxSize.height - 40, 120, 40, false);
    ...
    _camelPopViewModel = ShopPopViewModel(0, camelLeft, _lineBoxOffset.dy-32, 40, 36, false);
    double textLeft = 0;
    ...
    _camelTextPopViewModel = ShopPopViewModel(0, textLeft, 0, 110, 50, false, leftOrRight: leftOrRight);
    setState(() {});
  }
        ...
        children: <Widget>[
          Positioned(
            ...
            child: Offstage(
              ...
              child: Container(
                child: CustomPaint(
                  painter: ShopPagePopPainter(_popViewModel.startX, text: _popViewModel.popTitle, ),
                  size: Size(_popViewModel.width, _popViewModel.height),
                ),
              ),
            )
          ),
          _normalPopViewModel==null? Container():
           Positioned(
            left: _normalPopViewModel.left,
            top: _normalPopViewModel.top,
            ...
            child: Offstage(
              ...
              child: Container(
                child: CustomPaint(
                  painter: ShopPagePopPainter(_normalPopViewModel.startX, text: widget.camelPopTitle, ),
                  size: Size(_normalPopViewModel.width, _normalPopViewModel.height),
                ),
              ),
            )
          ),
          _camelPopViewModel==null? Container():
          Positioned(
            left: _camelPopViewModel.left,
            top: _camelPopViewModel.top,
            width: _camelPopViewModel.width,
            height: _camelPopViewModel.height,
            child: Offstage(
              offstage: _camelPopViewModel.offstage,
              child: Image.asset("assets/camel_white.png", fit: BoxFit.fill,),
            ),
          ),
          _camelTextPopViewModel==null?Container(): 
          Positioned(
            left: _camelTextPopViewModel.left,
            top: _camelTextPopViewModel.top,
            ...
            child: Offstage(
              ...
              child: Stack(
                children: <Widget>[
                  Image.asset(_camelTextPopViewModel.leftOrRight?"assets/camel_pop_left.png":"assets/camel_pop_right.png"),
                  Center(
                    child: Container(
                      padding: EdgeInsets.symmetric(horizontal: 20),
                      child: Text(widget.camelTitle, maxLines: 2, style: TextStyle(color: Colors.black38.withAlpha(140), fontSize: 10)),
                    )
                  )
                ],
              )
          ...
          // safawfawe
          Positioned(
            left: 0, right: 0, bottom: 41, height: 3,
            child: _progressWidget(),
          ),
          Positioned(
            // left: 0, right: 0, bottom: 40, height: 5,
            left: 0, right: 0, bottom: 22.5, height: 40,
            child: Row(
    ...
    return Container(
      ...
      child: Stack(
        children: <Widget>[
          ...
          Positioned(
            left: 0, top: 0, bottom: 0, width: widget.width*_percent,
            child: Container(
              decoration: BoxDecoration(
                color: Colors.white,
                borderRadius: BorderRadius.circular(2),
  ...
  _progressNodeWidget(nodeKey){
    return GestureDetector(
      onTap: (){
        ...
        if (widget.nodeTitleList!=null&&widget.nodeTitleList.length==widget.nodeCount) {
          _popViewModel.popTitle = widget.nodeTitleList[_keyList.indexOf(nodeKey)];
        }
