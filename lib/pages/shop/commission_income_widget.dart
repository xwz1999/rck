...
class _CommissionIncomeWidgetState extends State<CommissionIncomeWidget> {
  ...
  _popupMenuWidget(){
    return PopupMenuButton(
      color: Colors.white,
      offset: Offset(0, 100),
      child: Text(_selectValue, style: TextStyle(color: Colors.black),),
  ...
  _refreshWidget(){
    return RefreshWidget(
      ...
      body: _listData == null || _listData.length == 0 ? NoDataView(title:'没有数据...') : 
        ListView.builder(
          itemBuilder: (_, index){
  ...
  _getListModel() async {
    ResultData resultData = await HttpManager.post(ShopApi.income_sales_sum_list, {
            ...
            child: Column(
              children: <Widget>[
                ...
                Container(
                  ...
                  alignment: Alignment.centerLeft,
                  child: Text(data.orderTime!=null?data.orderTime:"", style:TextStyle(color: Colors.black.withOpacity(0.5),fontSize: ScreenAdapterUtils.setSp(12) ),)
                ),
                Spacer(),
                Container(
                  height: 0.3,
